...
static void draw_number(int x, int y, uint64_t value)
{
    char text[12];
    int length = 0;
    do { text[length++] = (char)('0' + value % 10); value /= 10; } while (value && length < 11);
    for (int i = 0; i < length / 2; i++) { char t = text[i]; text[i] = text[length - i - 1]; text[length - i - 1] = t; }
    text[length] = '\0';
...
void taskbar_draw(void)
{
    ...
    if (active && active->visible) {
        ...
        fb_draw_rect(74, height - 23, 145, 18, OR_COLOR_BORDER);
        fb_draw_string(80, height - 17, active->title, OR_COLOR_TEXT_DIM);
    }
    fb_draw_string(width - 86, height - 17, "TICKS", OR_COLOR_TEXT_DIM);
    draw_number(width - 50, height - 17, timer_get_ticks());
}
